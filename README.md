
# Engenharia de Sofwtare - Melhoria de Processo[

Escopo Funcional (Funcionalidades do Sistema)
* Cadastro de Usuários
* Cadastro de Impressoras
* Visualização de Impressoras no Mapa
* Listagem e Filtros de Impressoras
* Solicitação de Serviço de Impressão
* Sistema de Avaliação

Escopo Não Funcional
* Algumas funcionalidades serão tratadas como requisitos não funcionais ou simplificações do sistema.
* Pagamento
* Contato entre Usuários
* Segurança e Armazenamento de Arquivos


------------------- ------------------- ------------------- -------------------


Projeto dividido em fases para desenvolvimento e planejamento do produto

 Fases

* Iniciação
    - [x] Configuração do repositório
    - [x] Definição do escopo do Projeto   
    - [ ] Divisão e estrutura de artefatos
        *  1º - Requisitos
        * 2º - Análise e Design
        * 3º - Implementação
        * 4º - Teste
        * 5º - Implantação
        * 6º - Gerenciamento de Projeto
* Elaboração
* Construção
* Transição


## Tema Proposto - ???

## Qual Problema Resolveria?
